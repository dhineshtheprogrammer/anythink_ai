...
async def _mcp(
    ...
    if sub == "describe":
        if not rest:
            return CommandResult(error=True, message="Usage: /mcp describe <tool_name>")
        tool = mgr.get_tool(rest.strip())
        if tool is None:
            return CommandResult(
                error=True,
                message=f"Unknown tool '{rest.strip()}'. Use /mcp tools to list available tools.",
            )
        lines = [
            f"Tool:        {tool.name}",
            ...
            "",
            f"  {'Parameter':<20} {'Type':<10} Description",
            "  " + "─" * 60,
        ]
        if tool.input_schema:
            for param, meta in tool.input_schema.items():
                ptype = meta.get("type", "string")
                pdesc = meta.get("description", "")
                suffix = f"  (default: {meta['default']})" if "default" in meta else "  [required]"
                lines.append(f"  {param:<20} {ptype:<10} {pdesc}{suffix}")
        else:
            lines.append("  (no parameters)")
        arg_hints = " ".join(f"{k}=<value>" for k in (tool.input_schema or {}))
        lines.append("")
        lines.append(f"Usage: /mcp call {tool.name} {arg_hints}".rstrip())
        return CommandResult(message="\n".join(lines))
    ...
    return CommandResult(
        error=True,
        message="Usage: /mcp list|tools|describe|connect|disconnect|status|call|server|windows",
    )
